...
outcomes; specifically, we are interested in children's knowledge of letters, numbers, and forms.
...
site, sex, age, setting, and encouragement as other variables which could affect test scores. The dataset contained pre- and post-test scores for six different learning metrics, but
we were only interested in the scores for letters, numbers, and forms. We decided that the variables summarized in the table below were the most important to answer our client's research questions.
...
variable.desc <- data.frame(Name = c("ID", "Site", "Sex", "Age", "Viewcat",
				     "Setting", "Viewenc", "PAG", "Percent Improvement"))
variable.desc$Type <- c("Numerical", "Categorical",
			"Categorical","Numerical","Categorical","Categorical",
			"Categorical","Numerical", "Numerical")
variable.desc$Notes <- c("Identifying numeric sequence",
"Five different sampling sites (Explanatory)",
"Male or Female (Explanatory)",
"Age in months (Explanatory)",
"Categorical 1-4 encoding amount of show child watched (Explanatory)",
"Home or School (Explanatory)",
"Whether or not child was encouraged to view show (Explanatory)",
"Percent Acheivable Gain (Response)",
"The difference of post-score percent and pre-score percent (Response)")
...
leads for modeling, as well as concerns about possible data issues we will have to watch out for moving forward. We defined two possible response variables: percent improvement and percent achievable gain (PAG). Both involve the maximum score achievable for each test: 58 (letters), 20 (forms), and 54 (numbers). The first, percent improvement, is simply the difference between the posttest score and pretest score, expressed as percentages:
$$\text{PercentImprovement}=(\frac{\text{Post-test}}{\text{Max Score}}*100)-(\frac{\text{Pre-test}}{\text{Max Score}}*100)$$
The second, PAG, is the child's improvement (post-test score minus pre-test score) divided by
total possible improvement (maximum test score minus pretest score): 
$$\text{PAG}=\frac{\text{Post-test}-\text{Pre-test}}{\text{Max Score}-\text{Pre-test}}$$
The goal with this measure is to capture improvement while accounting for the fact that
...
research questions; then, we investigate possible confounders that could interfere with our analysis. 

```{r}
#| label: fig-LNF1
#| echo: false
#| warning: false
#| fig-cap: "The Relationship Between Viewing Time of Sesame Street and Percent Score Improvement for Letters, Numbers, and Forms. Note that viewing time is categorical with higher values corresponding to more time viewing Sesame Street"
#| layout-ncol: 3
#| fig-subcap:  
#|   - "Letters"
#|   - "Numbers"
...
between viewing time (categorized as levels 1-4) and percent score improvement.
Although the variability is not xcompletely consistent across viewing levels,
overall the data are relatively well-behaved. 

Due to the more well-behaved nature of percent improvement compared to PAG, that will
be the response variable we use in modeling. In the following investigation of possible
confounders, we will use only percent improvement. 
...
plot <- ggplot(longData, aes(x = site, y = diff_pct, fill = which_test)) + 
  ...
  xlab("Site") +
  ylab("Percent Improvement") + 
  labs(title = "Percent Improvement by Site", fill="Which Test") +
  scale_y_continuous(label=labels)
plot
```
The plot in @fig-Site shows some variation in score improvement across the
different sites for all three tests. Not only is median improvement in
...
will have to look out for issues with the non-uniform variability. 
...
plot <- ggplot(longData, aes(x = age, y = diff_pct, col = which_test)) + 
  ...
  xlab("Age") +
  ylab("Percent Improvement") + 
  labs(title = "Percent Improvement by Age", col = "Which Test") +
  scale_y_continuous(label=labels)
plot 
```
The plot in @fig-Age doesn't appear to show a strong relationship between age and
improvement in score in any of the tests. The points appear to be
scattered more or less at random. It remains unclear whether or not age will be an 
important confounder to in our models. 
...
In this table, we are most interested in the p-values on the right-hand column.
Although some values appear to be zero, they are not, but are so small that
they were rounded to zero. We will consider any p-value less than 0.05 to be
"significant," meaning we can infer that what we observe is a product of the
true process and not random chance. For the quantitative variables of age and
prescore, the small p-values indicate that each of these variables is important
for predicting improvement. For the categorical variables of viewcat and site,
the small p-values indicate that at least one of the viewing categories and at
least one of the sites resulted in a different improvement than another. The
same interpretations can be applied to the two ANOVA tables below, which are
analogous to this one but for the forms and numbers models. 
...
The relationship between subject depended primarily on viewing category. For
children who watched more Sesame street (viewcat 3 or 4), their smallest
improvements were in numbers and forms, while their greatest improvements were
in letters, after accounting for age and site.
...
relationship between viewing time and percent score improvement. However, by
...
#| echo: false
#| warning: false
#| fig-cap: "Comparing Percent Improvement Across Three Subjects: Letters, Numbers, and Forms"
#| layout-ncol: 1
...
plot <- ggplot(longData, aes(x=which_test, y=diff_pct, fill=which_test)) +
  ...
  xlab("Which Test") + 
  ylab("Percent Improvement") +
  labs(title = "Percent Improvement by Test") 
plot
...
#| echo: false
#| warning: false
#| fig-cap: "Comparing Percent Improvement Across Three Subjects (Letters, Numbers, and Forms) depending on Encouragement "
#| layout-ncol: 3
#| fig-subcap:  
#|   - "Letters"
#|   - "Numbers"
...
an impact on percent score improvement, particularly for letters, but there is
...
#| echo: false
#| warning: false
#| fig-cap: "Comparing Percent Improvement Across Three Subjects (Letters, Numbers, and Forms) depending on Sex "
#| layout-ncol: 1
...
plot <- ggplot(longData, aes(x = which_test, y = diff_pct, fill = sex)) + 
  ...
  xlab("Which Test") +
  ylab("Percent Improvement") + 
  labs(title = "Percent Improvement by Sex")
plot
```
...
#| echo: false
#| warning: false
#| fig-cap: "Comparing Percent Improvement Across Three Subjects (Letters, Numbers, and Forms) depending on Setting "
#| layout-ncol: 1
...
plot <- ggplot(longData, aes(x = which_test, y = diff_pct, fill = setting)) + 
  ...
  xlab("Which Test") +
  ylab("Percent Improvement") + 
  labs(title = "Percent Improvement by Setting") 
plot
```
...
Similarly to the evalutatory plots above, the plots in @fig-eval-mixed-model all look reasonable per our standards. The qqplot for the random effects does have some tailing on the left side, but this is not drastic enough to cause us a lot of concern. 
